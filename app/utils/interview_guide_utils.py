...
async def generate_interview_guide_with_gemini(
    ...
    if not settings.GEMINI_API_KEY:
        raise ValueError(
            "Chave da API do Gemini não configurada. Configure GEMINI_API_KEY no arquivo .env"
        )
    ...
    **REGRAS:**
    ...
    try:
        ...
        if (
            ...
            and response.candidates[0].finish_reason == "MAX_OUTPUT_TOKENS"
        ):
            logger.warning("Resposta do Gemini foi truncada devido ao limite de tokens")

        response_text = response.text.strip()
        logger.debug(
            f"Resposta bruta do Gemini (primeiros 500 chars): {response_text[:500]}"
        )
        ...
        error_msg = str(e)
        logger.error(
            f"ERRO ao gerar guia: Quota da API do Gemini excedida. {error_msg}"
        )
...
def extract_interview_json_from_text(text: str) -> dict:
    ...
    logger.debug(
        f"Texto limpo para extração JSON (primeiros 300 chars): {cleaned_text[:300]}"
    )
    ...
    try:
        ...
        if matches:
            ...
            logger.debug(
                f"JSON encontrado via regex (primeiros 300 chars): {json_str[:300]}"
            )
            try:
                return json.loads(json_str)
            except json.JSONDecodeError as e2:
                logger.error(
                    f"Erro no parse do JSON regex: {e2} (posição: {e2.pos if hasattr(e2, 'pos') else 'desconhecida'})"
                )
                ...
                try:
                    ...
                    try:
                        ...
                        if partial_json:
                            logger.warning(
                                "Usando JSON parcial extraído devido a erro de parsing"
                            )
                            return partial_json
        ...
        raise ValueError(
            f"Não foi possível extrair JSON válido do texto. Erro: {str(e)}"
        )
...
def repair_truncated_json(json_str: str, error: json.JSONDecodeError = None) -> str:
    ...
    repaired = re.sub(
        r'"\s*\n\s*"', '",\n"', repaired
    )  # Adiciona vírgula entre strings em múltiplas linhas
